        ...
        return regeneratorRuntime.wrap(function _callee$(_context) {
            while (1) {
                switch (_context.prev = _context.next) {
                    case 0:
                        _context.next = 2;
                        return (0, _nodeFetch2.default)('http://' + URL + '/').then(function (res) {
                            return res.text();
                        });

                    case 2:
                        html = _context.sent;
                        info = /"(http:\/\/www.dilidili.wang\/anime\/(\d{6})\/)".*target="_blank">(.*)<\/a>/.exec(html);
                        urls = [];
                        desc = [];
                    ...
                    case 2:
                        ...
                        return _context2.abrupt('return', arr.map(function (item) {
                            ...
                            if (tmp[1].indexOf('http') == -1) {
                                if (tmp[1].indexOf('/') == 0) tmp[1] = 'http://' + URL + tmp[1];else tmp[1] = 'http://' + URL + '/' + tmp[1];
                            }
                            return {
                                img: tmp[1],
        ...
        return regeneratorRuntime.wrap(function _callee3$(_context3) {
            while (1) {
                switch (_context3.prev = _context3.next) {
                    case 0:
                        return _context3.abrupt('return', Promise.all(urls.map(function (url) {
                            return getJson(url);
                        })));
